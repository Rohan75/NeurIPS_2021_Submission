...
def get_correlations(set1, set2):
    ...
    for i in range(len(vector_mesh)):
        r = stats.pearsonr(vector_mesh[i][0], vector_mesh[i][1])
        correlation_matrix.append(r[0])
    correlation_matrix = np.array(correlation_matrix).reshape((len(set1), len(set2)))
    
    return correlation_matrix

def get_correlation_map(feature_maps, position_interactions):
    num_filters = feature_maps.shape[2]
    
    filter_interactions = feature_maps[position_interactions].transpose([1, 2, 0])
    correlation_map = get_correlations(filter_interactions[0], filter_interactions[1])
    
    return correlation_map

def get_correlation_stats(fmaps, att_maps, thresh, filter_matches, expecteds, corr_thresh, negate=False, limit=100000, rand=0):
    
    pos_ints1 = get_position_interactions(att_maps, threshold=thresh, limit=int(limit*(1-rand)))
    pos_ints2 = [np.random.randint(len(att_maps), size=(int(limit*rand), 1)), np.random.randint(att_maps.shape[1], size=(int(limit*rand), 2))]
    position_interactions = [np.vstack([pos_ints1[0], pos_ints2[0]]), np.vstack([pos_ints1[1], pos_ints2[1]])]
    
    correlation_map = get_correlation_map(fmaps, position_interactions)
    correlation_map = absmaxND(np.array([correlation_map, correlation_map.transpose()]), axis=0)
    correlation_map = np.tril(correlation_map, k=-1)
    correlation_map = np.nan_to_num(correlation_map)
    if negate:
        correlation_map = correlation_map * -1

    filter_interactions = np.array(np.where(correlation_map > corr_thresh)).transpose()
    correlation_interactions = get_motif_interactions(filter_interactions, filter_matches)
    
    tpr, fpr, cov = get_interaction_stats(correlation_interactions, expecteds)
    
    return correlation_map, correlation_interactions, tpr, fpr, cov
...
def get_clustered_correlation_stats(clustered_fmaps, att_maps, group_names, expecteds, thresh, corr_thresh, negate=False, limit=100000):
    
    correlation_map, correlation_interactions, tpr, fpr, cov = get_correlation_stats(clustered_fmaps, att_maps, thresh, group_names, expecteds, corr_thresh, negate=negate, limit=limit)
