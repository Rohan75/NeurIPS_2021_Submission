...
def run_pipeline(model, baseline, category, variant, trial, motifs, batch_size=200, epochs=100):
    ...
    for i in range(len(thresholds)):
        ...
        for j in range(len(topNs)):
            ...
            if thresholds[i] == 0.1 and topNs[j] == 15:
                print(f'PPV {corr_tpr} | TPR {corr_cov}')
    ...
    stats = [correlation_tprs, correlation_covs, correlation_aurocs, correlation_auprs, correlation_snrs, significance, dinuc_attention_values, correlation_maps, tomtom_tpr, tomtom_fpr, tomtom_cov, auc_roc, auc_pr]
